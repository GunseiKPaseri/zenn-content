...
`C:\Windows\System32\OpenSSH`に存在する `OpenSSH`はやや古く、あまり更新される気配が有りませんので、必要に応じてバージョンアップ等します。
...
`.ssh`と `.ssh/authorized_keys`の所有者が `*USERNAME*`になっているか確認する。
...
先頭の `common-auth`のインクルードのコメントアウト、末尾にオーセンティケータの設定の追加します。
